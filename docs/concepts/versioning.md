...
```text
<helper-semver>-<restic-version>          # stable, e.g. 3.0.0-0.18.1
<helper-semver>-<restic-version>-dev      # testing, e.g. 3.0.0-0.18.1-dev
```
...
| **MAJOR** (`X+1.0.0`) | Breaking configuration, path, or runtime contract change. | `1.18.0 → 2.0.0` (`bisync` → `replicate` rename), `2.x → 3.0.0` (removal of `SYNC_*` + `/bin/bisync` bridge). |
...
So `3.0.0-0.18.1-dev` is "helper 3.0.0 on top of Restic 0.18.1, testing
...
## Historical: the 2.x replicate bridge

The `1.18 → 2.0` **replicate** rename shipped a **compatibility bridge**
for all of **2.x**: `/bin/bisync` symlinked to `/bin/replicate`, and `SYNC_*`
env vars mapped to `REPLICATE_*` at startup with deprecation warnings.

**3.0.0 removes that bridge** — only `REPLICATE_*` and `/bin/replicate`
remain. See [Upgrading → 2.x → 3.0.0](../getting-started/upgrading.md#replicate-30-bridge).
