...
| **MINOR** (`x.Y+1.0`) | New feature, new environment variable, new script hook or materially new behaviour. Drop-in compatible. | `2.0.0 → 2.1.0` (`/bin/doctor`), `2.1.0 → 2.2.0` (`/bin/snapshot-export`). |
...
│   └──────── helper PATCH (CI/doc fix)
└──────────── helper MAJOR.MINOR (replicate rename + snapshot-export)
```

So `2.2.2-0.18.1-dev` is "helper 2.2.2 on top of Restic 0.18.1, testing
