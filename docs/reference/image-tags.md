...
```text
<helper-semver>-<restic-version>          # stable, e.g. 2.13.0-0.18.1
<helper-semver>-<restic-version>-dev      # testing, e.g. 2.13.0-0.18.1-dev
```
...
| `<semver>-<restic>` | Pinned stable, e.g. `2.13.0-0.18.1`. |
...
Concrete example: `2.13.0-0.18.1-dev` is "helper 2.13.0 on top of Restic
...
- **OCI image labels**:
    ...
    org.opencontainers.image.version=2.13.0-0.18.1
    ```
...
docker run --rm marc0janssen/restic-backup-helper:2.13.0-0.18.1 \
