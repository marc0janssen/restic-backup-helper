...
```shell
trivy image --severity HIGH,CRITICAL marc0janssen/restic-backup-helper:2.11.0-0.18.1
```
...
# Build with SBOM:
SBOM=ON ./build.sh
# → sbom/restic-backup-helper-2.11.0-0.18.1.spdx.json
# → sbom/restic-backup-helper-2.11.0-0.18.1.cyclonedx.json
```
...
syft marc0janssen/restic-backup-helper:2.11.0-0.18.1 -o spdx-json > local.spdx.json
syft marc0janssen/restic-backup-helper:2.11.0-0.18.1 -o cyclonedx-json > local.cdx.json

# Diff against the one shipped in this repo's sbom/ (after ./build.sh):
diff <(jq -S . local.spdx.json) <(jq -S . sbom/restic-backup-helper-2.11.0-0.18.1.spdx.json)
```
...
```shell
docker pull marc0janssen/restic-backup-helper:2.11.0-0.18.1
docker inspect --format '{{ index .RepoDigests 0 }}' marc0janssen/restic-backup-helper:2.11.0-0.18.1
