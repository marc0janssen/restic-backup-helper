...
| `RESTIC_AUTO_UNLOCK` | `OFF` | When `ON`, `/bin/backup` and `/bin/check` run `restic unlock` after a non-zero restic exit. Default leaves the lock alone — safer for repositories shared across multiple hosts. Use the audited [`/bin/unlock`](../operations/unlock.md) helper for explicit, logged manual unlocks instead. Conceptual background: [Repository locks](../operations/repository-locks.md). |
...
| `BACKUP_CRON_MINUTE_STAGGER` | *(empty)* | When **`ON`**, `/entry.sh` replaces the **minute** field (first of five cron fields) before writing crontab: new minute = POSIX `cksum(id) % BACKUP_CRON_STAGGER_MODULO` (default `60`). **`id`** is `BACKUP_CRON_STAGGER_ID` if set, else `HOSTNAME`. The minute field must be `*` or a decimal `0`–`59`; other forms (`*/15`, lists, ranges) are **not** rewritten (stderr warning at startup). The effective expression is re-exported as `BACKUP_CRON` so diagnostics match cron. Spreads load when many hosts share one repository and the same nominal schedule. |
| `BACKUP_CRON_STAGGER_ID` | *(empty)* | Non-empty string hashed instead of `HOSTNAME` when `BACKUP_CRON_MINUTE_STAGGER=ON` (e.g. Kubernetes pod name from the downward API). |
| `BACKUP_CRON_STAGGER_MODULO` | `60` | Divisor for the stagger minute (`1`–`60`); default `60` yields minutes `0`..`59`. Invalid values fall back to `60` with a warning. |
