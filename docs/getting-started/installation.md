...
Tags follow the schema `<helper-semver>-<restic-version>`, e.g. `2.12.1-0.18.1`,
so you can pin **both** the helper logic and the Restic base image in lockstep.
Testing builds add a `-dev` suffix, e.g. `2.12.1-0.18.1-dev`.
...
=== "Testing"
    ...
    ```shell
    docker pull marc0janssen/restic-backup-helper:2.12.1-0.18.1-dev
    ```
