...
- **Observability**: each run writes `/var/log/last-{backup,check,prune,replicate,restore,snapshot-export,forget-preview,mount-snapshot,unlock,sources-report,init-repo}.json` and, when `WEBHOOK_URL` is set, POSTs the same JSON document to your monitoring endpoint (healthchecks.io, Slack, Discord, Gotify, ntfy, …).
- **Hooks**: optional `/hooks/{pre,post}-{backup,check,prune,replicate,restore,snapshot-export,forget-preview,mount-snapshot,unlock,sources-report,init-repo}.sh` scripts run before/after each job, with consistent start/exit-code/duration logging and an optional `HOOK_TIMEOUT`.
