...
- **Observability**: each run writes `/var/log/last-{backup,check,prune,replicate,restore,snapshot-export,forget-preview,mount-snapshot,unlock,sources-report}.json` and, when `WEBHOOK_URL` is set, POSTs the same JSON document to your monitoring endpoint (healthchecks.io, Slack, Discord, Gotify, ntfy, …).
- **Hooks**: optional `/hooks/{pre,post}-{backup,check,prune,replicate,restore,snapshot-export,forget-preview,mount-snapshot,unlock,sources-report}.sh` scripts run before/after each job, with consistent start/exit-code/duration logging and an optional `HOOK_TIMEOUT`.
...
| --- | --- | --- |
| **Stable** | Production | `docker pull marc0janssen/restic-backup-helper:latest` or pinned `marc0janssen/restic-backup-helper:2.8.0-0.18.1` |
| **Testing** | Pre-release / CI | `docker pull marc0janssen/restic-backup-helper:develop` or `marc0janssen/restic-backup-helper:2.8.0-0.18.1-dev` |
