...
- **Sources report**: run `/bin/sources-report` for a read-only pre-flight inventory: readability, type, file count and (optional) size per source, plus pattern counts and missing-entry counts for every `--files-from` / `--exclude-file` referenced in `RESTIC_JOB_ARGS`. Catches missing mounts, stale `--files-from` entries and silently-empty exclude files before the next backup runs. Supports `--no-size` for slow / remote sources, `--depth N` to cap traversal, plus repeatable `--source PATH` / `--files-from FILE` for ad-hoc inspection.
...
> - **From 2.10.0 → 2.10.1:** patch release. Prometheus textfile metrics now escape the `hostname` label before writing `restic_<job>.prom` and emit the documented `restic_<job>_last_started_timestamp` gauge, the shared webhook helper documentation now matches its return-code contract, and the docs clarify that `RESTIC_*_ARGS` / `REPLICATE_*_ARGS` values are whitespace-split argument strings, not a full shell parser. Keep paths and argument values free of spaces, or use file-based inputs such as `--files-from`, `--exclude-file` and rclone config files.
> - **From 2.9.0 → 2.10.0:** purely additive. New operator helper **`/bin/notify-test`** sends clearly-labelled test mail and/or webhook notifications through the same `notify_mail` / `notify_webhook` helpers used by real jobs, so operators can validate `msmtprc`, `MAILX_RCPT`, `WEBHOOK_URL`, `WEBHOOK_HEADER_AUTH` and `WEBHOOK_TIMEOUT` before waiting for a real failure. Default mode sends to every configured target; `--mail`, `--webhook` and `--all` select target scope; `--dry-run` prints what would be sent without invoking `mail` or `curl`; `--subject` / `--message` label the test. Unlike real workers, delivery failures affect the helper exit code (`1`) so CI can catch notification drift. Writes `/var/log/last-notify-test.json`, `restic_notify_test.prom`, and runs `pre-notify-test` / `post-notify-test` hooks. No new environment variables.
> - **From 2.8.0 → 2.9.0:** purely additive. New audited operator helper **`/bin/init-repo`** is the operator-driven counterpart to the entrypoint auto-init probe (`RESTIC_CHECK_REPOSITORY_STATUS`). The recommended deployment pattern on shared remotes is now `RESTIC_CHECK_REPOSITORY_STATUS=OFF` (no auto-init on a transient TLS / DNS / auth hiccup) plus a one-shot `/bin/init-repo --yes` (CI) or interactive `/bin/init-repo` (operator) for the first bootstrap. `--dry-run` runs the same `restic cat config` probe, prints the planned `restic init` command (with masked repository URL and resolved flags) and a one-line verdict (`would CREATE` / `would REFUSE — already exists` / probe error) without mutating anything. Without `--dry-run` the helper requires either a typed `init` confirmation (interactive TTY) or an explicit `--yes` / `-y` flag, so a container restart can never re-initialise a repository. Adds the **`RESTIC_INIT_ARGS`** env-var (whitespace-split, analogous to `RESTIC_FORGET_ARGS` / `RESTIC_PRUNE_ARGS`) for `--repository-version=2` / `--copy-chunker-params=…` pinning; CLI passthrough after `--` works too. Writes `/var/log/last-init-repo.json` (with `dry_run`, `assume_yes`, `confirmed`, `repo_existed`, `probe_exit_code`, `init_args`), `restic_init_repo.prom`, and runs `pre-init-repo` / `post-init-repo` hooks, mail and webhook the same way as the other workers. Reachable via `docker exec … /bin/init-repo` or `docker run … init-repo`. Idempotent: `restic cat config` exit 0 → exit `3` (no mutation); exit 10 → proceed; other exits → abort with exit `2`. No behaviour change for the entrypoint auto-init probe; `RESTIC_CHECK_REPOSITORY_STATUS=ON` keeps its existing semantics.
> - **From 2.7.0 → 2.8.0:** purely additive. New read-only operator helper **`/bin/sources-report`** is a pre-flight inventory of the paths your next backup will actually read: re-uses the same `BACKUP_ROOT_DIR` + `RESTIC_JOB_ARGS` parsing as `/bin/backup`, surfaces readability, type, file count and (optional) size per source, and additionally lists every `--files-from` / `--exclude-file` reference with pattern counts and missing-entry counts for the lines inside `--files-from` files. Default scope is automatic (everything `/bin/backup` would see); `--source PATH` and `--files-from FILE` add ad-hoc entries; `--no-size` skips `du -sk` / `find -type f` on slow / remote sources; `--depth N` caps `find` depth on huge trees. Emits `/var/log/last-sources-report.json` (with flat aggregates plus nested `sources`, `files_from`, `exclude_files` arrays), `restic_sources_report.prom`, plus the `pre-sources-report` / `post-sources-report` hook pair, mail and webhook — exactly like the other operator wrappers. Reachable via `docker exec … /bin/sources-report` or the entrypoint shortcut `docker run … sources-report`. The size figure is **unfiltered** (exclude rules are not applied); the exclude-file inventory is reported separately. No env-var changes. Internal refactor: `collect_arg_paths` (the `RESTIC_JOB_ARGS` parser) is promoted from `/bin/doctor` to `app/lib.sh` so both helpers tokenise `--files-from` / `--exclude-file` identically — pure refactor, no behaviour change for `/bin/doctor`.
...
> - **From 1.18.x → 2.0.0:** the old "sync/bisync" surface is renamed to **replicate**. Use `/bin/replicate`, `REPLICATE_*` env vars, `/config/replicate_jobs.txt`, `/hooks/pre-replicate.sh` / `/hooks/post-replicate.sh`, `/var/log/last-replicate.json`, `/var/log/replicate-last.log` and `restic_replicate.prom`. Through **2.x** only, legacy `SYNC_*` and `/bin/bisync` were still accepted with deprecation warnings (**removed in 3.0.0**). The old `config/sync_jobs.txt` sample is not installed anymore; rename your mounted file or set `REPLICATE_JOB_FILE` explicitly.
> - **From 1.14.x → 1.15.0:** purely additive. New opt-in env vars: `METRICS_DIR` (Prometheus textfile collector) and `REPLICATE_BISYNC_CHECK_ACCESS` (bisync `--check-access` opt-in; was also exposed as `SYNC_BISYNC_CHECK_ACCESS` in 1.x only). Mail subjects gain a richer prefix (`[OK|FAIL N] Backup …`); update any subject-based filter rules. Container logs now mask inline credentials in replicate source/destination URLs.
...
| `RESTIC_JOB_ARGS` | *(empty)* | Extra words passed to `restic backup` (whitespace-split; not full shell syntax). Examples: `--exclude-file /config/exclude_files.txt`, `--limit-upload 5000`. Keep argument values free of spaces; use file-based inputs such as `--files-from` / `--exclude-file` for complex path lists. |
| `RESTIC_FORGET_ARGS` | *(empty)* | If set **and** backup exits `0`, runs `restic forget` with these words (whitespace-split; not full shell syntax), e.g. `--retry-lock=5m --keep-daily 7 --keep-weekly 5 --keep-monthly 12`. Add `--prune` only if you do not run `PRUNE_CRON` separately. |
...
| `RESTIC_CHECK_ARGS` | *(empty)* | Extra arguments for `restic check` (whitespace-split; not full shell syntax). |
...
| `REPLICATE_JOB_FILE` | `/config/replicate_jobs.txt` | Job file: `SOURCE;DESTINATION[;MODE[;EXTRA_ARGS]]` per line (comments `#` allowed). See [Optional Rclone replicate jobs](#optional-rclone-replicate-jobs). |
| `REPLICATE_JOB_ARGS` | *(empty)* | Extra global args passed to every rclone job (whitespace-split; not full shell syntax; `--resync` is stripped from routine runs). Per-job extras can be added via the 4th column of `REPLICATE_JOB_FILE`; keep values free of spaces or move them into rclone config/files. |
...
| `/hooks/pre-mount-snapshot.sh` | Before `restic mount` is invoked |
| `/hooks/post-mount-snapshot.sh` | After the FUSE mount has been released; receives **mount snapshot exit code** as `$1` |
...
Use **`REPLICATE_*`** and **`/bin/replicate`** only. The old sample name `config/sync_jobs.txt` is not installed anymore; rename it to `config/replicate_jobs.txt` or set `REPLICATE_JOB_FILE` to the path you mount.
...
Always-emitted gauges (one of each per `<job>` ∈ `backup`, `check`, `prune`, `replicate`, `restore`, `snapshot_export`, `forget_preview`, `mount_snapshot`):
...
- **`RESTIC_JOB_ARGS` / `RESTIC_CHECK_ARGS` / `RESTIC_FORGET_ARGS` / `RESTIC_PRUNE_ARGS` / `RESTIC_INIT_ARGS` / `REPLICATE_JOB_ARGS`** — printed verbatim because they are caller-controlled. Avoid stuffing secrets into these (use `RESTIC_PASSWORD_FILE` and `--password-command` files instead).
...
`/bin/status` (alias `/bin/health-summary`) is the fast daily command
for "is this container broadly healthy?" It does **not** run `restic`,
`rclone`, hooks, mail, webhooks or repository probes; it only reads
local state: release metadata, rendered crontab (or env preview),
recent `/var/log/last-*.json` files and the ages / exit codes for
scheduled `backup`, `check`, `forget`, `prune` and `replicate` runs.
It prints a compact `OK` / `WARN` / `FAIL` summary and supports
`--json` with schema `restic-backup-helper.status/1`.

```shell
docker exec -ti restic-backup-helper /bin/status
docker exec restic-backup-helper /bin/status --json | jq '.verdict, .findings'
docker run --rm --env-file restic.env marc0janssen/restic-backup-helper:latest health-summary
```
...
- Masked effective environment values for `RESTIC_*`, `REPLICATE_*`, mail and webhook settings.
...
- Recent `/var/log/last-{backup,check,prune,forget,replicate,restore,snapshot-export,forget-preview,mount-snapshot,unlock,sources-report,init-repo,notify-test,restore-test}.json` summaries and the last 40 lines of `/var/log/cron.log`.
...
| `--policy ARGS` | Use these retention args instead of `RESTIC_FORGET_ARGS`; quote as one argument. The value is then whitespace-split like `RESTIC_FORGET_ARGS`, not parsed as full shell syntax. |
...
`/bin/mount-snapshot` wraps `restic mount` (FUSE) so operators can
browse any matching snapshot read-only without doing a full restore.
Defaults are designed for the common case "give me last night's
snapshot tree, scoped to this host":

- mounts on `/restore` by default,
- adds `--host "$HOSTNAME"` and `--tag "$RESTIC_TAG"` unless you pass
  `--repo-wide`,
- refuses to mount on `/data`, `BACKUP_ROOT_DIR` or other
  system/source directories unless `--force` is given (the FUSE mount
  would hide the backup source while active),
- traps `EXIT` and calls `fusermount -u` (with `umount` fallback) so
  Ctrl+C, SIGTERM or a restic crash always unmounts cleanly.

```shell
# Mount this host's snapshots read-only under /restore.
...
Browse the tree from a second shell while the mount is up:

```shell
docker exec -ti restic-backup-helper sh -c '
  ls /restore/snapshots/latest
  cp /restore/snapshots/latest/data/documents/report.pdf /tmp/report.pdf
'
```
...
| Flag | Purpose |
| --- | --- |
| `--target PATH` | Mountpoint (default: `/restore`). Created if missing; must be writable and empty (or pass `--force`). |
| `--tag TAG` | Filter visible snapshots by tag (default: `RESTIC_TAG`). Ignored with `--repo-wide`. |
| `--host HOST` | Filter visible snapshots by host (default: container `HOSTNAME`). Ignored with `--repo-wide`. |
| `--path PATH` | Only expose snapshots that include this path (repeatable). |
| `--repo-wide` | Do not add host/tag filters; expose every snapshot in the repository. |
| `--allow-other` | Pass restic's `--allow-other` (requires `user_allow_other` in `/etc/fuse.conf`). |
| `--force` | Allow mounting on a non-empty target or refused path. |

The helper writes `/var/log/mount-snapshot-last.log`,
`/var/log/last-mount-snapshot.json`, optional hooks
`/hooks/pre-mount-snapshot.sh` and
`/hooks/post-mount-snapshot.sh "$rc"`, webhooks/mail through the existing
notification helpers, and `restic_mount_snapshot.prom` when `METRICS_DIR`
is set.

FUSE inside the container still requires `--cap-add SYS_ADMIN --device
/dev/fuse` (or the Kubernetes equivalents). See the full documentation
at <https://marc0janssen.github.io/restic-backup-helper/operations/mount-snapshot.html>.
...
docker exec -ti restic-backup-helper /bin/snapshot-export --id latest
docker exec -ti restic-backup-helper /bin/forget-preview
docker exec -ti restic-backup-helper /bin/mount-snapshot          # mount snapshots read-only under /fusemount (Ctrl+C unmounts)
...
**FUSE mount inside the container** (browse snapshots) — prefer
`/bin/mount-snapshot` (see [Mount snapshot](#mount-snapshot)) which adds
safe target validation, host/tag scope, observability and a clean
unmount trap on top of `restic mount`:

```shell
docker exec -ti restic-backup-helper /bin/mount-snapshot
```

The raw equivalent for environments without the helper:
...
- Helm chart: [`charts/restic-backup-helper/`](charts/restic-backup-helper/)
